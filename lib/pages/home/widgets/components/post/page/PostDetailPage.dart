    ...
    try {
      ...
      final selectedReportType = await showDialog<Map<String, dynamic>>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: reportTypes.map((reportType) {
              return ListTile(
      ...
      if (selectedReportType != null && mounted) {
        try {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(response['message'] ?? 'Post reported successfully'),
              ),
            );
          }
        } catch (e) {
  ...
  Future<void> _showReportCommentDialog(String commentId) async {
    try {
      ...
      if (selectedReportType != null && mounted) {
        try {
          // Send report to the backend
          final response = await _apiClient.post('/api/report/post-comment', {
            'commentId': commentId,
            'reportType': selectedReportType['id'],
            'reason': selectedReportType['name'],
          });
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ListTile(
                                            ...
                                            onTap: () {
                                              Navigator.pop(context);
                                              _showReportCommentDialog(commentId);
                                            },
                                          ),
                                        ],
                                      ),
