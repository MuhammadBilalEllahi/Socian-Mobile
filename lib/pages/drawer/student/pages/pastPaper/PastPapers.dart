...
class _PastPapersState extends State<PastPapers> {
  ...
  // Mapping of report type labels to ObjectId values (replace with actual ObjectIds from your backend)
  final Map<String, String> reportTypeIds = {
    'Inappropriate Content': '507f1f77bcf86cd799439011',
    'Spam': '507f1f77bcf86cd799439012',
    'Incorrect Paper': '507f1f77bcf86cd799439013',
  ...
  Widget _reportReasonDialog(String pastPaperItemId) {
    final isDarkMode = Theme.of(context).brightness == Brightness.dark;
    return AlertDialog(
      title: const Text('Report Past Paper', style: TextStyle(fontSize: 16)),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
        ...
        TextButton(
          onPressed: () {
            if (_selectedReportType != null &&
                _reportReasonController.text.trim().isNotEmpty) {
              Navigator.pop(context, {
                'reportType': reportTypeIds[_selectedReportType]!,
                'reason': _reportReasonController.text.trim(),
              });
            } else {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                _showSnackBar('Please select a report type and enter a reason',
                    isError: true);
              });
            }
          },
  ...
  Future<void> _handleReport(String pastPaperItemId) async {
    _selectedReportType = null;
    _reportReasonController.clear();
    final result = await showDialog<Map<String, String>>(
      context: context,
      builder: (context) => _reportReasonDialog(pastPaperItemId),
    );
    if (result == null) return;
    try {
      final response = await apiClient.post(
        '/api/report/past-paper-item',
        {
          'pastPaperItemId': pastPaperItemId,
      ...
      if (response['success'] == true) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          _showSnackBar(
              response['message'] ?? 'Failed to report past paper item',
              isError: true);
        });
      }
    } catch (e) {
