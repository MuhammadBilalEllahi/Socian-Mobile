...
class _CommentItemState extends ConsumerState<_CommentItem> {
  ...
  Future<void> _handleEdit(Map<String, dynamic> comment) async {
    final parentState =
        context.findAncestorStateOfType<TeacherMainPageCommentsState>();
  ...
  Future<void> _showReportDialog(String feedbackId) async {
    try {
      final apiClient = ApiClient();
      ...
      if (selectedReportType != null && mounted) {
        try {
          final response = await apiClient.post('/api/report/feedback', {
          ...
          debugPrint('Error reporting feedback: $e');
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  Row(
    children: List.generate(5, (index) {
      final rating = (widget.comment['rating'] is int)
          ? (widget.comment['rating'] as int).toDouble() // Convert int to double
          : widget.comment['rating'] as double; // Already double
